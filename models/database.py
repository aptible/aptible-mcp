...
class DatabaseManager(ResourceManager[Database, str]):
    ...
    async def create(self, handle: str, account_id: str, image_id: int) -> Database:
        """
        ...
        response = self.api_client.post(f"/databases/{database.id}/operations", operation_data)
        self.api_client.wait_for_operation(response["id"])
        
        return database
    
    async def delete_by_handle(self, handle: str, account_id: Optional[str] = None) -> bool:
        """
        ...
        if not database and account_id:
            ...
            databases = [db for db in all_databases if db.handle == handle and db.account_id == account_id]
            if databases:
