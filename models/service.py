...
class Service(ResourceBase):
    ...
    container_count: int = Field(..., description="Number of containers running this service")
    container_memory_limit_mb: int = Field(..., description="Size of containers running this service in MB")
...
class ServiceManager(ResourceManager[Service, str]):
    ...
    async def list_by_app(self, app_id: str) -> list[Service]:
        """
        List all services for a specific app.
        """
        response = self.api_client.get(f"/apps/{app_id}/services?per_page=5000&no_embed=true")
        items = response["_embedded"][self.resource_name]
        return [self.resource_model.model_validate(item) for item in items]

    async def get_by_handle_and_app(self, handle: str, app_id: str) -> Optional[Service]:
        """
    ...
    async def scale(self, service_id: int, container_count: int | None, container_memory_limit_mb: int | None) -> Service:
        """
        ...
        response = self.api_client.post(f"/services/{service_id}/operations", operation_data)
