...
class VhostManager(ResourceManager[Vhost, str]):
    ...
    async def create(self, service_id: str) -> Vhost:
        """
        ...
        response = self.api_client.post(f"/vhosts/{vhost.id}/operations", operation_data)
        self.api_client.wait_for_operation(response["id"])
        
        return vhost
    
    async def list_by_service(self, service_id: str) -> List[Vhost]:
        """
        List all vhosts for a specific service.
        """
        response = self.api_client.get(f"/services/{service_id}/vhosts?per_page=5000&no_embed=true")
        items = response["_embedded"][self.resource_name]
        return [self.resource_model.model_validate(item) for item in items]

    async def delete_vhost(self, vhost_id: str) -> None:
        """
        ...
        response = self.api_client.post(f"/vhosts/{vhost_id}/operations", operation_data)
        self.api_client.wait_for_operation(response["id"])
