...
from models import (
    AccountManager,
    App, AppManager,
    Database, DatabaseManager, StackManager,
    VhostManager,
)
...
async def create_account(account_name: str, stack_name: str) -> Dict[str, Any]:
    ...
    account = await account_manager.create(account_name, stack.id)
...
@mcp.tool()
async def get_app(app_handle: str, account_handle: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    Get app by handle.
    ...
    if account_handle:
        ...
        if not account_matches:
            raise Exception(f"No app with handle {app_handle} in account {account_handle}.")
        
        return account_matches[0].model_dump()
    
    raise Exception(f"Multiple apps found with handle {app_handle}. Please provide an account handle.")


@mcp.tool()
async def create_app(app_handle: str, account_handle: str, docker_image: str) -> Dict[str, Any]:
    """
    Create a new app.
    """
    account = await account_manager.get(account_handle)
    ...
    app = await app_manager.create(app_handle, account.id, docker_image)
    return app.model_dump()


@mcp.tool()
async def configure_app(app_handle: str, account_handle: Optional[str], env: Dict[str, str]) -> None:
    """
...
@mcp.tool()
async def get_database(database_handle: str, account_handle: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    ...
    if account_handle:
        ...
        if not account_matches:
            raise Exception(f"No database with handle {database_handle} in account {account_handle}.")
        
        return account_matches[0].model_dump()
    
    raise Exception(f"Multiple databases found with handle {database_handle}. Please provide an account handle.")


@mcp.tool(description="Create a new database. The image_id should be the ID found via list_available_database_types.")
async def create_database(database_handle: str, account_handle: str, image_id: int) -> Dict[str, Any]:
    """
    ...
    database = await database_manager.create(database_handle, account.id, image_id)
    return database.model_dump()


@mcp.tool()
async def delete_database(database_handle: str, account_handle: Optional[str] = None) -> None:
    """
    Delete a database.
    """
...
async def create_vhost(app_handle: str, service_handle: str, account_handle: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    if not service_matches:
        raise Exception(f"No service with handle {service_handle} found in app {app_handle}.")
    
    service = service_matches[0]
    vhost = await vhost_manager.create(service.id)
    return vhost.model_dump()


@mcp.tool("Delete a Vhost (aka Endpoint) by ID.")
async def delete_vhost(vhost_id: str) -> None:
    """
...
@mcp.tool()
async def list_services(app_handle: str, account_handle: Optional[str] = None) -> List[Dict[str, Any]]:
    """
...
@mcp.tool()
async def get_service(app_handle: str, service_handle: str, account_handle: Optional[str] = None) -> Dict[str, Any]:
    """
...
async def scale_service(
    ...
    if container_count is None and container_memory_limit_mb is None:
        raise ValueError("Must specify at least one of container_count or container_size")
        
    service_data = await get_service(app_handle, service_handle, account_handle)
    service = Service.model_validate(service_data)

    updated_service = await service_manager.scale(service.id, container_count, container_memory_limit_mb)
    return updated_service.model_dump()


@mcp.tool("List all vhosts for a service.")
async def list_service_vhosts(app_handle: str, service_handle: str, account_handle: Optional[str] = None) -> List[Dict[str, Any]]:
    """
